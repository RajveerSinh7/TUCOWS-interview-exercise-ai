...
2. **User-uploaded docs** via `optional.py` (placed in `support_docs/` folder), which are converted into the proper JSON format automatically and indexed.

Both approaches ensure MCP-compliant outputs for seamless integration into support workflows.
...
- **Config (**``**)** – Environment vars for API keys, models, and paths.
- **Index Builder (**``**)** – Embeds hardcoded docs and builds FAISS index.
- **Optional Doc Uploader (**``**)** – Converts user-uploaded `.txt` files in `support_docs/` into the correct JSON format and builds index.
- **Vector Store (**``**)** – Loads index and retrieves relevant docs.
- **Prompting Engine (**``**)** – Constructs MCP-structured prompts with role, context, and JSON schema.
- **LLM Client (**``**)** – Mistral API wrapper with fallback for dev/demo.
- **API Layer (**``**)** – FastAPI endpoints for single/batch resolution, with validation and error handling.
...
- **Option 1: Hardcoded docs** (quick demo)

```bash
python src/index_builder.py
```

- **Option 2: User-uploaded docs** (place `.txt` files in `support_docs/` folder)
